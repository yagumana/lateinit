...
# データフォルダの作成
output_dir = f'./data/3d_jacobi_sq_{grid_size}/pdf2'
...
# ヤコビアン行列の行列式を計算する関数
def compute_jacobian_determinant_3d_2(data, mask, dim):
    ...
    det_jacobian_product = np.zeros((grid_size, grid_size, grid_size))
    for i in range(grid_size):
        for j in range(grid_size):
            for k in range(grid_size):
                if not mask[i * grid_size**2 + j * grid_size + k]:
                    continue
                jacobian_transpose = np.transpose(jacobian[i, j, k])
                jacobian_product = np.dot(jacobian_transpose, jacobian[i, j, k])
                det_jacobian_product[i, j, k] = np.linalg.det(jacobian_product)

    return det_jacobian_product


# 最初のフレームからdet_jacobianの範囲を取得
initial_det_jacobian = compute_jacobian_determinant_3d_2(residual_stack[0], mask, dim).flatten()[mask]
...
# # 全体のdet_jacobianの範囲を取得
# det_jacobian_all = np.concatenate([
#     compute_jacobian_determinant_3d_2(residual_stack[i], mask, dim=3).flatten()[mask]
#     for i in tqdm(range(timesteps_cnt))
# ])
# # ファイルに保存
# np.save(f'/workspace/score_data/det_jacobi2_all_{grid_size}.npy', det_jacobian_all)

# # ファイルから読み込み
# det_jacobian_all = np.load(f'/workspace/score_data/det_jacobi2_all_{grid_size}.npy')
...
# カラーバーの設定
norm = Normalize(vmin=0, vmax=vmax*3)
sm = plt.cm.ScalarMappable(cmap=cm.viridis, norm=norm)
sm.set_array([])
# colorbar = fig.colorbar(sm, ax=ax, shrink=0.5, aspect=5, label='Jacobian^2 Determinant')
...
for num in tqdm(range(timesteps_cnt)):
    ...
    det_jacobian = compute_jacobian_determinant_3d_2(residuals, mask, dim=3)
    ...
    # 画像を保存
    plt.savefig(os.path.join(output_dir, f'jacobi2_{num:04d}.pdf'))
