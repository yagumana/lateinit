...
# 2次元の円周上の点の集合からなるデータセットを定義

from torch.utils.data import TensorDataset

def circle_dataset2(n=8000):
...
dataset = circle_dataset2(8000)
dataloader = DataLoader(dataset, batch_size=32, shuffle=True, drop_last=True)
model = MLP(hidden_size=128, hidden_layers=3, emb_size=128, time_emb="sinusoidal", input_emb="sinusoidal", out_dim=2)
noise_scheduler = NoiseScheduler(num_timesteps=1000, beta_schedule="linear")
optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3)


checkpoint = torch.load('/workspace/weights/2d_points_weights0.pth')
...
file_path = f'./score_data/residual_stack_2d_{grid_size}.npy'
...
# データフォルダの作成
output_dir = '/workspace/data/2d_jacobian_squared_re/pdf'
...
# サンプルデータを作成します。ここではresidual_stackが既に生成されています。
data_set = residual_stack

# ヤコビアン行列の行列式を計算する関数
def compute_jacobian_determinant(data, grid_size, dim):
    grid_data = data.reshape(grid_size, grid_size, dim)
    jacobian = np.zeros((grid_size, grid_size, dim, dim))

    for i in range(1, grid_size - 1):
        for j in range(1, grid_size - 1):
            dx = (grid_data[i + 1, j] - grid_data[i - 1, j]) / 2
            dy = (grid_data[i, j + 1] - grid_data[i, j - 1]) / 2
            jacobian[i, j, :, 0] = dx  # ∂f/∂x
            jacobian[i, j, :, 1] = dy  # ∂f/∂y

    for i in [0, grid_size - 1]:
        for j in range(grid_size):
            jacobian[i, j] = jacobian[1, j] if i == 0 else jacobian[grid_size - 2, j]
    for j in [0, grid_size - 1]:
        for i in range(grid_size):
            jacobian[i, j] = jacobian[i, 1] if j == 0 else jacobian[i, grid_size - 2]

    # det(J^T * J) を計算する
    det_jacobian_product = np.zeros((grid_size, grid_size))
    for i in range(grid_size):
        for j in range(grid_size):
            jacobian_transpose = jacobian[i, j].T
            jacobian_product = np.dot(jacobian_transpose, jacobian[i, j])
            det_jacobian_product[i, j] = np.linalg.det(jacobian_product)

    return det_jacobian_product

# 最初のステップでdet_jacobianの最小値と最大値を計算
initial_det_jacobian = compute_jacobian_determinant(data_set[0], grid_size, dim)
...
for i in tqdm(range(len(data_set))):
    det_jacobian = compute_jacobian_determinant(data_set[i], grid_size, dim)
    img = ax.imshow(det_jacobian, cmap='viridis', interpolation='nearest', extent=[-3, 3, -3, 3], vmin=vmin, vmax=vmax)
    ...
    # 画像を保存
    plt.savefig(os.path.join(output_dir, f'jacobian_squared_{i:04d}.pdf'))
